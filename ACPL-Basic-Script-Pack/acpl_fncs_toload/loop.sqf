...
acpl_loop = {
	...
	_arty_rounds = ["acpl_arty_rounds", "Iloœæ pocisków", "acpl_icons\arty.paa", {}, {_player getvariable "acpl_arty_fo"}] call ace_interact_menu_fnc_createAction;
	...
	_arty_correction = ["acpl_arty_correction", "Ustaw poprawkê", "acpl_icons\arty.paa", {}, {(_player getvariable "acpl_arty_fo") AND !(_player getvariable "acpl_arty_iscalling") AND ([_player] call acpl_arty_check_lastfire)}] call ace_interact_menu_fnc_createAction;
	
	_arty_correction_0_0 = ["acpl_arty_correction_0_0", "Wyzeruj poprawkê", "acpl_icons\arty.paa", {
	...
	_arty_correction_n = ["acpl_arty_correction_n", "Pó³noc", "acpl_icons\arty.paa", {}, {(_player getvariable "acpl_arty_fo") AND (count (_player getVariable "acpl_arty_lastfire") > 0) AND !(_player getvariable "acpl_arty_iscalling") AND ([_player] call acpl_arty_check_lastfire)}] call ace_interact_menu_fnc_createAction;
	...
	_radio_lower_sw = ["acpl_radio_lowerheadset_sw", "Zdejmij s³uchawki (SW)", "acpl_icons\radio.paa", {private ["_volume"];_volume = (call TFAR_fnc_ActiveSwRadio) call TFAR_fnc_getSwVolume;_player setvariable ["acpl_radio_volume_sw",_volume,true];_player setvariable ["acpl_radio_lower_sw",true,true];[(call TFAR_fnc_ActiveSWRadio), 1] call TFAR_fnc_setSwVolume;hint "Zdj¹³eœ s³uchawki z radia krótkiego";}, {(call TFAR_fnc_haveSWRadio) AND !(_player getvariable "acpl_radio_lower_sw")}] call ace_interact_menu_fnc_createAction;
	_radio_onhead_sw = ["acpl_radio_headset_sw", "Za³ó¿ s³uchawki (SW)", "acpl_icons\radio.paa", {_player setvariable ["acpl_radio_lower_sw",false,true];[(call TFAR_fnc_ActiveSWRadio), (_player getvariable "acpl_radio_volume_sw")] call TFAR_fnc_setSwVolume;hint "Za³o¿y³eœ s³uchawki z radia krótkiego";}, {(call TFAR_fnc_haveSWRadio) AND (_player getvariable "acpl_radio_lower_sw")}] call ace_interact_menu_fnc_createAction;
	_radio_lower_lr = ["acpl_radio_lowerheadset_lr", "Zdejmij s³uchawki (LR)", "acpl_icons\radio.paa", {private ["_volume"];_volume = (call TFAR_fnc_ActiveLrRadio) call TFAR_fnc_getLrVolume;_player setvariable ["acpl_radio_volume_lr",_volume,true];_player setvariable ["acpl_radio_lower_lr",true,true];[(call TFAR_fnc_ActiveLrRadio), 1] call TFAR_fnc_setLrVolume;hint "Zdj¹³eœ s³uchawki z radia d³ugiego";}, {(call TFAR_fnc_haveLRRadio) AND !(_player getvariable "acpl_radio_lower_lr")}] call ace_interact_menu_fnc_createAction;
	_radio_onhead_lr = ["acpl_radio_headset_lr", "Za³ó¿ s³uchawki (LR)", "acpl_icons\radio.paa", {_player setvariable ["acpl_radio_lower_lr",false,true];[(call TFAR_fnc_ActiveLRRadio), (_player getvariable "acpl_radio_volume_lr")] call TFAR_fnc_setLrVolume;hint "Za³o¿y³eœ s³uchawki z radia d³ugiego";}, {(call TFAR_fnc_haveLRRadio) AND (_player getvariable "acpl_radio_lower_lr")}] call ace_interact_menu_fnc_createAction;
	_radio_ask_sw = ["acpl_radio_ask_sw", "Poproœ o po¿yczenie radia (SW)", "acpl_icons\radio.paa", {
		if (isPlayer _target) then {
			["Zosta³eœ poproszony o po¿yczenie radia (SW)"] remoteExec ["hint",_target];
	...
	_radio_ask_lr = ["acpl_radio_ask_lr", "Poproœ o po¿yczenie radia (LR)", "acpl_icons\radio.paa", {
		if (isPlayer _target) then {
			["Zosta³eœ poproszony o po¿yczenie radia (LR)"] remoteExec ["hint",_target];
	...
	while {true} do {
		
		{
			...
			if (!(_x in acpl_static_done) AND _static) then {
				...
				_action = ["acpl_ai_action", "Odblokuj AI", "acpl_icons\run.paa", {{[_x,"move"] remoteExec ["enableAI",0];} foreach (units (group _player));hint "Odblokowa³eœ AI w swojej grupie";}, {_player == leader (group _player)}] call ace_interact_menu_fnc_createAction;
